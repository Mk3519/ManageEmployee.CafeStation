...
function getEmployeesByBranch(branch) {
  try {
    ...
    console.log('Branch received:', branch); // للتحقق من الفرع المستلم
    console.log('Total rows:', data.length); // للتحقق من عدد الصفوف

    const employees = data.slice(1)
      .filter(row => {
        console.log('Checking row:', row); // للتحقق من كل صف
        console.log('Branch in row:', row[4]); // للتحقق من قيمة الفرع
        return row[4] === branch;
      })
      .map(row => ({
        code: row[0],
    ...
    console.log('Filtered employees:', employees); // للتحقق من النتيجة النهائية
...
function recordAttendance(attendanceData) {
  try {
    ...
    attendanceData.forEach((record, index) => {
      if (!record.date || !record.employeeId || !record.status) {
        throw new Error(`Invalid record data at index ${index}`);
      }
    });
    ...
    return ContentService.createTextOutput(JSON.stringify({
      success: true,
      message: `تم تسجيل الحضور بنجاح لـ ${attendanceData.length} موظف`
    })).setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    console.error('Error in recordAttendance:', error);
    return ContentService.createTextOutput(JSON.stringify({
